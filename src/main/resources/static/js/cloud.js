$(document).ready(function () {
    var path = [];
    var folderId = 0;
    var optFileIdList = [];//被选中的文件（夹）id列表，用于文件移动
    var fileIdList = [];
    var pageFileCount =  (Math.floor($("#main_right_content").height()/110)+1) * Math.floor($("#main_right_content").width()/110);
    getUserInfo();

    $("#main_right_content").scroll(function(){
        var nDivHight = $("#main_right_content").height();
    ...
    $("#btnUploadSubmit").click(function () {
        ...
        data.append("folderId", Number(path[path.length - 1].id));
        uploadFile(data);
    ...
    $("#btnNewFolderSubmit").click(function () {
        var data = JSON.stringify({
                folderId: Number(path[path.length - 1].id),
    ...
    function setFileList(data) {
        ...
        $.each(data, function (i, item) {
            var fIsFolder = item.fileType === "folder";
    ...
    function showInfo(data) {
        ...
        htmlStr += "<tr><td><b>创建时间:</b></td><td>" + data.createTime + "</td></tr>";
